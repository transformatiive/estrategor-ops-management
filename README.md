...
- [x] **TRNSF‑966 · Limpeza da navegação lateral:** removido o grupo *Programas* (PT2030/Formação/Fiscal) da sidebar — redundante com o filtro de programa na listagem (TRNSF‑965); a pill do programa permanece em cada projeto. A sidebar fica **Geral** (Dashboard, Projectos, Prazos, Tarefas) + **Configuração**.
- [x] **TRNSF‑965 · Listagem de Projectos (filtros + lista/kanban):** barra de filtros (pesquisa, programa, fase, responsável, família) aplicável a ambas as vistas, e **toggle lista/kanban** (preferência recordada). O **kanban** tem seletor de secção (Candidatura | Execução), 5 colunas por secção em **linguagem de cliente** com contagem; cartões mostram cliente, família e sinal de estado. RBAC no `GET /api/projects` (consultor vê os seus, gestor todos) e `family` no DTO.
- [x] **TRNSF‑964 · Dashboard de Trabalho:** o dashboard deixa de ser um placar de contadores (saiu o "estado da API/uptime") e passa a um **painel por ação** — saudação com a tese do dia, cartões *À minha espera / A aguardar cliente / Prazo esta semana / Em execução*, e secções: **À minha espera** (projetos onde o consultor é o bloqueio, com o *que falta* lido do pipeline/Verificador), **A aguardar o cliente** (documentos em falta + estado do lembrete, link para a Recolha) e **carteira por fase**. Consultor vê os seus; gestor vê todos com filtro por consultor.
- [x] **TRNSF‑963 · Vista de Pipeline na página de projeto:** a barra de separadores plana dá lugar a um **pipeline em linguagem de cliente** (Diagnóstico → Recolha → Preparação → Revisão → Submissão, e bloco Execução esbatido "se aprovado") — sem códigos A0–A4/Fase A/B. Cada passo reflete o estado real da máquina de estados; bloco **"o que falta para avançar"** com os pré‑requisitos da fase atual (validação 942 + Verificador 946); as **vistas mostradas são só as relevantes** à fase. Badge de estado em linguagem de cliente.
- [x] **TRNSF‑962 · Internacionalização · Intake diferenciado:** ramo Internacionalização do formulário ao cliente (TRNSF‑937) — recolhe ações por evento/país, mercados‑alvo, RH a contratar, certificações e estratégia. As respostas entram com `origem='intake'` e pré‑preenchem `cand_intl_acoes` (960), `cand_intl_rh` (961) e os mercados‑alvo; o consultor revê no preview. Aparece quando a candidatura é de Internacionalização.
- [x] **TRNSF‑961 · Internacionalização · Detalhe da Ação + RH a Contratar (B.10/B.11):** por ação — custos por rubrica (catálogo de feiras/ações), sub‑tabela de deslocações (pessoa/destino/dias/viagem/estadia/ajudas) e RH a contratar (função/custo/período). Totais agregados com **coerência** contra o custo da componente financeira (TRNSF‑944). Caracterização de cada ação via TRNSF‑943.
- [x] **TRNSF‑960 · Internacionalização · Ações de Intervenção + Indicadores (B.8/B.9):** 6 domínios fixos (catálogo TRNSF‑953), cada um aplicável com contributo; tabela densa de ações (domínio, tipo, mercado/país, ano) com linhas adicionáveis; indicadores com códigos do catálogo que alimentam o MP (946). Painel no separador *Candidatura* (só família Internacionalização).
- [x] **TRNSF‑954 · Exportação estruturada da Candidatura:** gera o pacote **Excel** (tabelas: balanço/DR/financiamento, indicadores, custos, mercado), **Word** (campos de texto com sub‑rótulos) e **PDF** (vista consolidada + relatório do Verificador: MP previsto + não‑conformidades) a partir dos dados validados. Antes de exportar avisa sobre campos por validar/`[A PREENCHER]`; os ficheiros são guardados na subpasta *Candidatura* do WorkDrive e descarregados para copy‑paste no portal SGO.
- [x] **TRNSF‑959 · Inovação · Intake diferenciado:** ramo Inovação do formulário ao cliente (TRNSF‑937) — recolhe intenções de investimento, tipologias pretendidas, indicadores‑meta e contexto. As respostas entram com `origem='intake'`/`por_validar` e pré‑preenchem `cand_investimentos` (945), `cand_tipologias` (955) e `cand_indicadores` (956); o consultor revê no preview. Aparece quando a candidatura é da família Inovação.
- [x] **TRNSF‑958 · Inovação · Substituição de Importações + Descrição Física (A.7/A.15):** secções condicionais — tabela de substituição de importações (produto/mercado, valor importado vs. produção nacional prevista, ligada às linhas de mercado) e descrição física do investimento (instalações, área, tipologia construtiva), relevante p.ex. para Turismo.
- [x] **TRNSF‑957 · Inovação · Indústria 4.0 + Transição Climática (A.16/A.17):** secções condicionais — Indústria 4.0 (4 âmbitos: produto/processo/organizacional/marketing) e Transição Climática (âmbitos do aviso); textos gerados via TRNSF‑943. Quando há indicadores **RPA**, a fundamentação dos indicadores RPA passa a obrigatória e é assinalada pelo Verificador (TRNSF‑946).
- [x] **TRNSF‑956 · Inovação · Atividades de Inovação + Indicadores (A.11/A.12):** lista de atividades de inovação (caracterização gerável via TRNSF‑943) e tabela de indicadores com **códigos oficiais do catálogo** (TRNSF‑953), valor pré/meta e unidade; os calculáveis são sugeridos a partir da componente financeira (TRNSF‑944, `fonte=calculado`), os restantes por intake. Alimentam o cálculo do MP (TRNSF‑946).
- [x] **TRNSF‑955 · Inovação · Tipologias de Investimento (A.10):** secção *Tipologias* (família Inovação) com as 4 tipologias e sub‑tabelas condicionais; os limiares regulamentares (≥ 20% de aumento de capacidade; investimento ≥ 200% dos ativos reutilizados na diversificação) são validados em código e assinalados. Fundamentação por tipologia via motor de geração (TRNSF‑943).
- [x] **TRNSF‑946 · Verificador + Cálculo de Mérito:** acção *Verificar* no separador *Candidatura* que devolve não-conformidades em **três eixos** — formulário (campos por gerar, `[A PREENCHER]`, limites, anexos obrigatórios), mérito e coerência interna (investimento = custos = financiamento, balanço, GAF×A0) — cada uma ligada à secção em falta. O **MP previsto** é calculado **deterministicamente** a partir da grelha (TRNSF‑941) e dos indicadores (TRNSF‑944), por critério A/B/C/D, com o mínimo do aviso; sem grelha, **avisa em vez de inventar**. Inclui a vista "Critérios de seleção". Resultado persistido (`verificacao`).
